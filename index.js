...
function sortByKey(arr, key) {
  ...
  return arr; // Використовуємо метод `sort` передаємо в нього два аргументи a та b, для сортування масиву об'єктів за заданим ключем
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (!Array.isArray(arr)) {
    return false;
  } else if (start < arr.length && start < 0 && end < arr.length && end < 0) {
    return false;
  } else {
